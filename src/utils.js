...
export const aggregateDataByIncomeSources = (data) => {
  const aggregatedData = data.reduce((result, item) => {
    ...
    const existingSource = result.find(
      (obj) => obj["Income sources"] === incomeSource
    );
    if (existingSource) {
  ...
  const sortedData = aggregatedData.sort((a, b) =>
    b["Income sources"].localeCompare(a["Income sources"])
  );

  const modifiedData = sortedData.map((item) => ({
    "Income sources": item["Income sources"],
    "Процентное соотношение - Counts": `${Math.floor(
      (item.Counts /
        aggregatedData.reduce((total, obj) => total + obj.Counts, 0)) *
        100
    )}%`,
    Counts: item.Counts.toLocaleString(undefined, { maximumFractionDigits: 0 }),
  }));

  const advertisingIndex = modifiedData.findIndex(
    (item) => item["Income sources"] === "Advertising"
  );
  const assetSaleIndex = modifiedData.findIndex(
    (item) => item["Income sources"] === "Asset sale"
  );

  if (advertisingIndex !== -1 && assetSaleIndex !== -1) {
    [modifiedData[advertisingIndex], modifiedData[assetSaleIndex]] = [
      modifiedData[assetSaleIndex],
      modifiedData[advertisingIndex],
    ];
  }
...
export function calculateAverageIncome(data, aggregateDataByMonth) {
  ...
  return averageIncome.toLocaleString();
}
...
export const calculateTotalOperatingProfit = (data) => {
  ...
  return aggregatedData.map((item) => ({
    Month: item.Month,
    "operating profit": item["operating profit"].toLocaleString(undefined, {
      maximumFractionDigits: 0,
    }),
  }));
};

export function getTotalOperatingProfitForYear(
  data,
...
export const aggregateDataByMarketingStrategies = (data) => {
  const totalIncome = data.reduce(
    (sum, item) => sum + parseFloat(item.Income),
    0
  );
  ...
  const aggregatedData = data.reduce((result, item) => {
    ...
    const existingStrategy = result.find(
      (obj) => obj["Marketing Strategies"] === strategy
    );
    if (existingStrategy) {
      existingStrategy.Income += income;
      existingStrategy["Процентное соотношение - Income"] =
        ((existingStrategy.Income / totalIncome) * 100).toFixed(2) + "%";
    } else {
      result.push({
  ...
  const modifiedData = aggregatedData.map((item) => ({
    ...
    Income: Math.round(item.Income).toLocaleString(undefined, {
      maximumFractionDigits: 0,
    }),
...
export const calculateIncomeRatio = (data) => {
  ...
  const totalTargetIncome = data.reduce(
    (sum, item) => sum + parseFloat(item["Target Income"]),
    0
  );
...
export const aggregateIncomeBySource = (data) => {
  ...
  const aggregatedData = Object.entries(incomeBySource).map(
    ([source, income]) => ({
...
export const aggregateDataByIncomeSourcess = (data) => {
  ...
  const incomeSourcesData = data.reduce((result, item) => {
    ...
    result[incomeSource].id = window.crypto.randomUUID();
    result[incomeSource].income += income;
    result[incomeSource].percentage =
      ((result[incomeSource].income / totalIncome) * 100).toFixed(2) + "%";
  ...
  const incomeBreakdownsData = data.reduce((result, item) => {
    ...
    result[incomeSource][incomeBreakdown].id = window.crypto.randomUUID();
    result[incomeSource][incomeBreakdown].income += income;
    result[incomeSource][incomeBreakdown].percentage =
      (
        (result[incomeSource][incomeBreakdown].income / totalIncome) *
        100
      ).toFixed(2) + "%";
